...
export async function GET(_request, { params }) {
  ...
  const { rows } = await query(
    `select ${COLS}, extracted_json as extracted from ops.legal_agreement where id = $1`, [id]
  );
...
export async function PATCH(request, { params }) {
  ...
  const cur = (await query('select extracted_json from ops.legal_agreement where id=$1', [id])).rows[0];
